...
- fig2_full_cohort_metrics.csv      scheme, event, event_lbl, phecode_id, phecode_ids,
                                    ...
                                    (with embed_surv_utils.find_icd_code as fallback), and a cohort
...
from _figure_utils import (
    DATA_PATH, FEATURE_PATH, RESULTS_PATH, SURV_PATH,
...
# code_data/ holds code lookups regenerated by this pipeline before figure data
# are prepared.
CODE_PATH = os.path.join(DATA_PATH, "code_data")
PROJECT_ROOT = Path(__file__).resolve().parents[3]
CODE_LOOKUP_GENERATORS = (
    PROJECT_ROOT / "python_scripts/data_preprocessing/generate_icd10_to_phecode_mapping.R",
    PROJECT_ROOT / "python_scripts/data_preprocessing/generate_phecode_descriptions.R",
)
...
def _initialize_code_lookups() -> None:
    """Regenerate and load all code-description inputs used by Figure 2."""
    for script in CODE_LOOKUP_GENERATORS:
        if not script.is_file():
            raise FileNotFoundError(f"Code lookup generator not found: {script}")
        print(f"  [code lookups] running {script.name}")
        try:
            subprocess.run(["Rscript", str(script)], check=True)
        except FileNotFoundError as exc:
            raise RuntimeError(
                "Rscript is required to generate Figure 2 code lookups"
            ) from exc
        except subprocess.CalledProcessError as exc:
            raise RuntimeError(
                f"Code lookup generator failed ({script.name}, exit {exc.returncode})"
            ) from exc
