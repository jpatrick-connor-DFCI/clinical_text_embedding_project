...
def _pooled(n, offset, seed, quartile="Q4"):
    ...
    return pl.DataFrame({
        ...
        "text_risk_score": rng.normal(size=n) + offset,
        "outer_fold": [0] * n,
        "stage_group": [["I", "II", "III", "IV"][s - 1] for s in stage],
        "stage_ordinal": stage,
        "risk_quartile": [quartile] * n,
    })


def test_pan_cancer_quartiles_are_kept_and_small_types_are_recorded():
    ...
    # Pan-cancer quartiles from the pooled table are kept, not re-cut within Breast.
    assert km["risk_quartile"].to_list() == ["Q4"] * 80
