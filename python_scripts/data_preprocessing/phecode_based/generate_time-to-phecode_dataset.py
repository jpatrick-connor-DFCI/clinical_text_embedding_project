...
def _normalize_phecode(code: str) -> Optional[str]:
    ...
    # Keep a single decimal point and strip trailing decimal zeros for stable matching.
    if code.count('.') > 1:
...
phecodes_to_analyze = (
    endpoint_df[endpoint_phecode_col]
    .map(_normalize_phecode)
    .dropna()
    .drop_duplicates()
    .tolist()
)
...
# Load ICD10 -> phecode mapping (all ICD variants)
...
if mapping_df.empty:
    raise ValueError(
        f"No rows remained after filtering {mapping_file} to phecodes in {endpoint_file}."
    )
...
# Load EHR ICD records and map to phecodes
split_ehr_icd_subset = pd.read_csv(os.path.join(SURV_PATH, 'timestamped_icd_info.csv'))
split_ehr_icd_subset['ICD10_NORM'] = split_ehr_icd_subset['DIAGNOSIS_ICD10_CD'].map(_normalize_icd10_undotted)
split_ehr_icd_subset = split_ehr_icd_subset.dropna(subset=['ICD10_NORM'])

split_ehr_icd_subset = split_ehr_icd_subset.merge(
...
# Generate time-to-event features for only requested phecode endpoints
for phecode in tqdm(phecodes_to_analyze, desc="Generating phecode events"):
    phecode_data_sub = split_ehr_icd_subset.loc[split_ehr_icd_subset['PHECODE'] == phecode]
...
event_cols = [event for event in phecodes_to_analyze if event in vte_data_sub.columns]
