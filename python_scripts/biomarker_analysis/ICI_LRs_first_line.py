...
ICI patients are defined by presence in IO_START.csv AND having ICI as their
first line of therapy (LINE == 1 with HAS_ICI in ALL_MEDICATION_LINES.csv).
...
# Paths
ICI_DATA_PATH = os.path.join(DATA_PATH, 'treatment_prediction/first_line_ICI_prediction_data/')
ICI_PROP_PATH = os.path.join(DATA_PATH, 'treatment_prediction/first_line_ICI_propensity/')
...
# --- Identify first-line ICI patients from ALL_MEDICATION_LINES.csv ---
med_lines_df = pd.read_csv('/data/gusev/USERS/mjsaleh/profile_lines_of_rx/ALL_MEDICATION_LINES.csv')
med_lines_df = med_lines_df.rename(columns={'MRN': 'DFCI_MRN'})
first_line_ici_mrns = set(
    med_lines_df.loc[
        (med_lines_df['LINE'] == 1) & (med_lines_df['HAS_ICI'] == 1),
        'DFCI_MRN'
    ].unique()
)
# Restrict to IO_START patients who also have first-line ICI
ici_mrns_first_line = ici_mrns & first_line_ici_mrns
...
# --- Classify patients ---
# ICI: first-line ICI patients in IO_START that are also in the survival cohort
ici_mrns_in_cohort = ici_mrns_first_line & cohort_mrns
...
print(f"First-line ICI (IO_START ∩ first-line ∩ cohort): {len(ici_mrns_in_cohort)}")
print(f"ICI in IO_START but not first-line:              {len((ici_mrns & cohort_mrns) - ici_mrns_first_line)}")
