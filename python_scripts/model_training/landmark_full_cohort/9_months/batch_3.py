...
tt_mets = [f'tt_{met_event}' for met_event in met_events]
...
twelve_month_events_df = pd.read_csv(os.path.join(LANDMARK_PATH, 'time_decayed_events_df_plus_12_months.csv'),
                                    usecols=['DFCI_MRN'] + events + tt_events + tt_mets)
...
met_free_12mo_mrns = (twelve_month_events_df.loc[(twelve_month_events_df[tt_mets] > 0).all(axis=1), "DFCI_MRN"].dropna().unique().tolist())
for event in tqdm(met_events):
    event_path = os.path.join(OUTPUT_PATH, event)
    os.makedirs(event_path, exist_ok=True)
    
    event_pred_df = full_prediction_df.loc[full_prediction_df['DFCI_MRN'].isin(met_free_12mo_mrns)].copy()
