...
- fig2_full_cohort_metrics.csv      scheme, event, event_lbl, phecode_id, text_cindex, base_cindex,
                                    ...
                                    events, the ICD-10 description for icd3_post/icd4_post codes via
                                    embed_surv_utils.find_icd_code, the phecode description for
                                    phecode_post codes via code_data/phecode_descriptions.csv (raw
                                    code if missing). phecode_id is the phecode identifying each
...
# code_data/ holds static code lookups generated by
# python_scripts/data_preprocessing/generate_icd10_to_phecode_mapping.R and
# generate_phecode_descriptions.R (not regenerated by this script).
...
# Static code lookups (generated on the cluster by python_scripts/data_preprocessing/
# generate_icd10_to_phecode_mapping.R and generate_phecode_descriptions.R). Loaded once
# at import time; both are optional so this script still runs (with raw-code fallback
# labels / no cross-scheme dedup) before those generator scripts have been run.
def _load_phecode_descriptions() -> dict[str, str]:
    ...
    return dict(zip(df["phecode"], df["description"]))


def _load_icd10_to_phecode() -> dict[str, str]:
    """ICD-10 code (undotted, e.g. 'C787') -> its primary mapped phecode."""
    ...
    df = df.dropna(subset=["icd10_code", "phecode"])
    out: dict[str, str] = {}
    for icd_code, phecode in zip(df["icd10_code"], df["phecode"]):
        for plen in (3, 4):
...
def _event_phecode(scheme: str, event: str) -> str | None:
    ...
    if scheme == "phecode_post":
        return event
    if scheme in ("icd3_post", "icd4_post"):
        return ICD10_TO_PHECODE.get(event)
    return None
