...
- **Disease-severity characteristics (Fig 4C).** Panel C is a 1×5 small-multiples row (% Stage IV, % ICI treated, mean # metastatic sites, **10-yr RMST** in months, **mean risk slope**) reading `fig4_cluster_severity.csv`. RMST (`restricted_mean_survival_time`, **τ=120 mo** = 5 y past the 60-mo landmark entry requirement, so RMST does not saturate at the entry cap). Stage and ICI tokens accept float repr (`4.0`) and the long form `Immune Checkpoint Inhibitors`. The mean-slope panel is the quantity the groups are defined on and is intentionally left un-clamped (no 0–100 range) since it is negative for the Falling group.
