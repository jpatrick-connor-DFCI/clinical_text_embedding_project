...
- **Disease-severity characteristics (Fig 4C).** Panel C is a 1×5 small-multiples row (% Stage IV, % ICI treated, mean # metastatic sites, **10-yr RMST** in months, **mean risk slope**) reading `fig4_cluster_severity.csv`. RMST uses `restricted_mean_survival_time` with **τ=120 mo**. Stage and ICI tokens accept float repr (`4.0`) and the long form `Immune Checkpoint Inhibitors`. The mean-slope panel is the quantity the groups are defined on and is intentionally left un-clamped (no 0–100 range) since it is negative for the Falling group.
...
- **Slope-group-count selection (Fig S1, appendix).** `prep_figure_4._silhouette_scan` computes silhouette vs k (2–8) on the standardized **slope** feature (not the full trajectory matrix); `figS1a` plots it and marks the chosen k = `N_SLOPE_GROUPS` = 3. Composed as `figureS1_cluster_silhouette` (compose key `s1`).
...
- **Within-stage risk-dynamics KM supplement (`plot_figure_4_supp.R`).** Two panels — Stage IV (A) and Stage I (B) — each a conditional-on-L KM (left-truncated `entry=L`, `x = L–120 mo`, same machinery as Fig 4B) stratified by risk-dynamics group, showing the Falling/Stable/Rising groups separate survival *within* a single clinical stage. Reads `fig4_km_data.csv`, which carries `stage` (major stage I–IV from `_major_stage_map` in `prep_figure_4.py`, `NaN` if unavailable) and the constant `landmark_month` so R does not hard-code L. The script defines its own `GROUP_NAMES`/`GROUP_COLORS`/`cluster_label` locally and a left-truncation-aware log-rank (`coxph` score test, since `survdiff` rejects `Surv(start, stop, event)`). Single-metric (no `MANUSCRIPT_METRIC` switch); composed as `figureS_stage_stratified_dynamics`.
