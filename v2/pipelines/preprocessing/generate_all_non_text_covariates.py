"""Build non-text clinical, genomic, and treatment covariates for the VTE cohort."""
...
def main() -> None:
    ...
    # === Cancer stage ===
    mrn_stage_dict = pickle.load(open(STAGE_PATH, 'rb'))
    ...
    cancer_type_sub.to_csv(os.path.join(FEATURE_PATH, 'cancer_type_df.csv.gz'), index=False)
    mrn_stage_df.to_csv(os.path.join(FEATURE_PATH, 'cancer_stage_df.csv.gz'), index=False)
    ...
    px_metadata = (pd.read_csv(os.path.join(DIAGNOSTICS_PATH, 'SOMATIC_SPECIMEN.csv'),
    ...
    complete_mutation_data.to_csv(os.path.join(FEATURE_PATH, 'complete_somatic_data_df.csv.gz'), index=False)

    prs_df = (pd.read_csv(PRS_MATRIX_FILE, sep='\t')
              .rename(columns={'IID' : 'cbio_sample_id'})
              .merge(px_metadata_min[['cbio_sample_id', 'DFCI_MRN']], on='cbio_sample_id'))
    prs_df.to_csv(os.path.join(FEATURE_PATH, 'complete_germline_data_df.csv.gz'), index=False)
    ...
    sv_data = pd.read_csv(
        os.path.join(DIAGNOSTICS_PATH, 'SOMATIC_SV_RESULTS.csv'),
    ...
    treatment_df = (pd.read_csv(MED_LINES_FILE)
    ...
    line_by_line_treatment_data.to_csv(os.path.join(FEATURE_PATH, 'categorical_treatment_data_by_line.csv.gz'), index=False)
    ...
    final_mean_lab_df.to_csv(os.path.join(FEATURE_PATH, 'mean_lab_vals_pre_first_treatment.csv.gz'), index=False)
