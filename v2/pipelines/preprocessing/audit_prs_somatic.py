...
def audit_somatic(cohort_df: pl.DataFrame) -> None:
    ...
    print(
        "\nIdmap-necessity check: v2's somatic path keys on DFCI_MRN natively from\n"
    ...
    v2_somatic_mrns = argmin_mrns
    only_in_idmap_path = idmap_mrns & set(cohort_mrns_list) - v2_somatic_mrns
    only_in_v2 = v2_somatic_mrns - idmap_mrns
    print(
        f"  v2 somatic-eligible cohort MRNs not in the 2024 idmap at all: {len(only_in_v2)} "
        "(these are patients the idmap-mediated path would have silently excluded).\n"
        f"  Cohort+idmap MRNs not reached by v2's somatic selection (different filter, not identity): {len(only_in_idmap_path)}."
    )
...
def report_related_findings() -> None:
    ...
    print(
        ...
        "- Two copies of assert_schema exist (v2/data/schema.py and\n"
        "  v2/pipelines/preprocessing/schema.py), functionally equivalent, selected by a\n"
