...
| 04d | [`04d_run_mortality_trajectories.ipynb`](04d_run_mortality_trajectories.ipynb) | cluster CPU | Landmark mortality risk trajectories (months 0–60) from `pipelines.trajectories.generate_mortality_trajectories`, with landmark coverage and at-risk denominators. **Many hours, but resumable** — each landmark is checkpointed as it completes, so an interrupted run picks up where it left off. Must run **before** 06b — `figures.prep.figure4` clusters these trajectories. |
