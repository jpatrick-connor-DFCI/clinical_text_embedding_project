...
#   "small" (stage, treatment, labs, somatic) : --cpus-per-task=1 --mem=4G
# because run_feature_comp_task.py already forces n_jobs=1 for the four small modalities
...
submit_class () {
    ...
    sbatch \
        ...
        --cpus-per-task="$cpus" \
        --mem="$mem" \
        --export=ALL,PROJECT_ROOT="$PROJECT_ROOT",MANIFEST="$MANIFEST",ROWS_PER_TASK="$ROWS_PER_TASK",MODALITY_CLASS="$class" \
...
if [[ "$MODALITY_CLASS" == "split" ]]; then
    ...
    sbatch \
        --array="0-${MAX_TASK}" \
        --export=ALL,PROJECT_ROOT="$PROJECT_ROOT",MANIFEST="$MANIFEST",ROWS_PER_TASK="$ROWS_PER_TASK",MODALITY_CLASS="$MODALITY_CLASS" \
        "$PROJECT_ROOT/v2/slurm/array_feature_comp.sh"
fi
